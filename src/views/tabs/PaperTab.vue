...
import {fetchPaperMcVersions, fetchPaperBuilds, fetchPaperBuildDetail} from '@/api'
...
const fetchVersions = async (): Promise<void> => {
  loading.value = true
  try {
    const allVersions = await fetchPaperMcVersions()
...
const fetchBuilds = async (): Promise<void> => {
  ...
  loading.value = true
  try {
    const data = await fetchPaperBuilds(selectedMcVersion.value)
...
const fetchBuildDetail = async (): Promise<void> => {
  ...
  loading.value = true
  try {
    buildDetail.value = await fetchPaperBuildDetail(selectedMcVersion.value, selectedBuild.value)
  } catch (e: any) {
    emit('error', e.message || '未知错误', e.message?.includes('CORS') || e instanceof TypeError)
  } finally {
    loading.value = false
