...
<template>
  <div class="paper-container">
    <n-space vertical size="large">
      ...
      >
        <template #content>
          <div><strong>项目:</strong> Paper</div>
...
import type {PaperBuildDetail, ServerType} from '@/types'
...
const type: ServerType = 'paper'
const minecraftVersions = ref<string[]>([])
const builds = ref<number[]>([])
const buildDetail = ref<PaperBuildDetail | null>(null)
...
const fetchVersions = async (): Promise<void> => {
  loading.value = true
  try {
    const resp = await fetch('https://fill.papermc.io/v3/projects/paper')
    if (!resp.ok) throw new Error('无法加载 Paper 版本列表')
...
const fetchBuilds = async (): Promise<void> => {
  ...
  loading.value = true
  try {
    const resp = await fetch(`https://fill.papermc.io/v3/projects/paper/versions/${selectedMcVersion.value}`)
    if (!resp.ok) throw new Error('无法加载 Paper 构建号')
    const data = await resp.json()
...
const fetchBuildDetail = async (): Promise<void> => {
  ...
  loading.value = true
  try {
    const resp = await fetch(`https://fill.papermc.io/v3/projects/paper/versions/${selectedMcVersion.value}/builds/${selectedBuild.value}`)
    if (!resp.ok) throw new Error('无法加载 Paper 构建详情')
...
const handleDownload = async (onProgress: (progress: DownloadProgress) => void): Promise<void> => {
  ...
  const filename = downloadInfo.name || `paper-${selectedMcVersion.value}-${selectedBuild.value}.jar`
  try {
